...
int16_t* fbDelayBuffer = nullptr;  
int16_t* freezeBuffer = nullptr;   
...
volatile bool isPB2WiperMode = false; 
...
volatile int latestPB3 = 2048;
volatile int latestBat = 2048;
...
void goToLightSleep() {
    ...
    while (digitalRead(BOOT_SENSE_PIN) == HIGH) {
        ...
        if (abs((int)latestPB1 - initA) > 150) break;
        if (abs((int)latestPB2 - initB) > 150) break;
        if (abs((int)latestPB3 - initC) > 150) break;
    }
...
void drawCircularGauge(TFT_eSprite& sprite, int x, int y, int radius, float value, const char* label, const char* valStr, uint32_t color, int type) {
    ...
    if (type == 1) { 
        ...
        if (sweep > 0) {
            sprite.drawArc(x, y, radius, radius - thickness, 0, sweep, color, TFT_BLACK, true);
        } else if (sweep < 0) {
            sprite.drawArc(x, y, radius, radius - thickness, 360 + sweep, 360, color, TFT_BLACK, true);
        }
    } else if (type == 2) { 
    ...
    sprite.drawString(label, x, y + 14);
}
...
void updateDisplay() {
    ...
    spr.setTextSize(1); 
    spr.drawString(batStr, 5, 2); 

    spr.setTextDatum(TR_DATUM); 
    if (btmidi.isConnected()) { 
        spr.setTextColor(TFT_GREEN, TFT_BLACK); 
        spr.drawString("BT: CONN", 315, 2); 
    } else { 
        spr.setTextColor(TFT_YELLOW, TFT_BLACK); 
        spr.drawString("BT: WAIT", 315, 2); 
    }
    ...
    spr.setTextSize(2); 
    spr.setTextColor(effectTitleColors[renderMode], TFT_BLACK); 
    spr.drawString(effectTitleNames[renderMode], 160, 15);
    
    bool effectIsActive = false;
    if (renderMode == 0) effectIsActive = isWhammyActive; 
    else if (renderMode == 1) effectIsActive = (isWhammyActive || isFrozen);
    else if (renderMode == 2) effectIsActive = (isWhammyActive || isFeedbackActive); 
    else if (renderMode == 3) effectIsActive = (isWhammyActive || isHarmonizerMode);
    else if (renderMode == 4) effectIsActive = (isWhammyActive || isCapoMode); 
    else if (renderMode == 5) effectIsActive = (isWhammyActive || isSynthMode);
    else if (renderMode == 6) effectIsActive = (isWhammyActive || isPadMode); 
    else if (renderMode == 7) effectIsActive = (isWhammyActive || isChorusMode);
    else if (renderMode == 8) effectIsActive = (isWhammyActive || isSwellMode); 
    else if (renderMode == 9) effectIsActive = (isWhammyActive || isVibratoMode);
    
    spr.fillCircle(240, 15, 6, effectIsActive ? TFT_GREEN : TFT_RED); 
    spr.drawCircle(240, 15, 6, TFT_WHITE);
    ...
    int bY = 115; 
    int bR = 17; 
    uint32_t actCol = effectTitleColors[renderMode];
    ...
    auto drawBanner = [&](const char* lbl, uint32_t col) { 
        spr.setTextColor(col, TFT_BLACK); 
        spr.drawString(lbl, bannerX, 150); 
        bannerX += 28; 
    };
    
    if (isFrozen && renderMode != 1) drawBanner("FRZ", TFT_CYAN); 
    if (isFeedbackActive && renderMode != 2) drawBanner("SCM", TFT_RED);
    if (isHarmonizerMode && renderMode != 3) drawBanner("HRM", TFT_MAGENTA); 
    if (isCapoMode && renderMode != 4) drawBanner("CAP", TFT_GREEN);
    if (isSynthMode && renderMode != 5) drawBanner("SYN", TFT_YELLOW); 
    if (isPadMode && renderMode != 6) drawBanner("PAD", TFT_PINK);
    if (isChorusMode && renderMode != 7) drawBanner("CHO", TFT_SKYBLUE); 
    if (isSwellMode && renderMode != 8) drawBanner("SWL", TFT_WHITE);
    if (isVibratoMode && renderMode != 9) drawBanner("VIB", TFT_PURPLE); 
        ...
        if (bytesRead > 0) {
            ...
            if (framesRead > 0) {
                ...
                if (globalAudioResetRequested) {
                    ...
                    inputEnvelope = 0.0f; 
                    feedbackFilterVar = 0.0f; 
                    smoothedVolGain = volumePedalGain; 
                    currentPitch = 1.0f;
                    freezeWriteIdxVar = 0; 
                    freezePlayCounterVar = 0; 
                    freezeStartIdxVar = 0; 
                ...
                if (audioBufferMutex != NULL && xSemaphoreTake(audioBufferMutex, 0) == pdTRUE) {
                    for(int j=0; j<10; j++) {
                        for(int k=0; k<5; k++) c_fx[j][k] = fxParams[j][k];
                    }
                    c_lat = latencyMode;      c_act = activeEffectMode;
                    c_w = isWhammyActive;     c_fz = isFrozen;
                    c_fb = isFeedbackActive;  c_hr = isHarmonizerMode;
                    c_cp = isCapoMode;        c_sy = isSynthMode;
                    c_pd = isPadMode;         c_ch = isChorusMode;
                    c_sw = isSwellMode;       c_vb = isVibratoMode;
                    c_pt = pitchShiftFactor;  c_vg = volumePedalGain;
                    xSemaphoreGive(audioBufferMutex);
                }
                ...
                float p_w_dry = c_fx[0][0]; 
                float p_w_wet = c_fx[0][1];
                float p_fz_apf = c_fx[1][0]; 
                float p_fz_att = c_fx[1][1]; 
                float p_fz_rel = c_fx[1][2];
                float p_fb_spd = c_fx[2][0]; 
                float p_fb_drv = c_fx[2][1]; 
                float p_fb_off = c_fx[2][2];
                float p_hr_mix = c_fx[3][0];
                float p_sy_att = c_fx[5][0]; 
                float p_sy_rel = c_fx[5][1]; 
                float p_sy_flt = c_fx[5][2]; 
                float p_sy_mix = c_fx[5][3];
                float p_pd_sm = c_fx[6][0]; 
                float p_pd_mix = c_fx[6][1];
                float p_ch_spd = c_fx[7][0]; 
                float p_ch_mix = c_fx[7][1];
                float p_sw_thr = c_fx[8][0]; 
                float p_sw_att = c_fx[8][1]; 
                float p_sw_rel = c_fx[8][2];
                float p_vb_dep = c_fx[9][0];
                ...
                if (frzActive && !wasFrozen) { 
                    ...
                    for (int s = 0; s < 4000; s++) {
                        ...
                        if (freezeBuffer[tempIdx] >= 0 && freezeBuffer[prev] < 0) { 
                            bestStart = tempIdx; 
                            break; 
                        }
                        tempIdx = prev;
                ...
                bool synthActive = ((c_act == 5 && c_w) || c_sy);
                bool padActive = ((c_act == 6 && c_w) || c_pd);
                bool harmActive = ((c_act == 3 && c_w) || c_hr);
                bool swellActive = ((c_act == 8 && c_w) || c_sw);
                bool chorusActive = ((c_act == 7 && c_w) || c_ch);
                
                bool feedbackActive = ((c_act == 2 && c_w) || c_fb);
                ...
                bool vibratoActive = ((c_act == 9 && c_w) || c_vb);
                bool capoActive = ((c_act == 4 && c_w) || c_cp);
                
                float peakInputVal = 0.0f; 
                float peakOutputVal = 0.0f;
                float localSwellGain = swellGain; 
                float localVolGain = c_vg;
                ...
                float g_whammy = c_w ? p_w_wet : 0.0f;
                float g_dry = c_w ? p_w_dry : 1.0f;
                ...
                for (int i = 0; i < framesRead; i++) {
                    ...
                    if (swellActive) {
                        localSwellGain = (inputEnvelope > p_sw_thr) ? __builtin_fminf(1.0f, localSwellGain + (p_sw_att * srScale)) : __builtin_fmaxf(0.0f, localSwellGain - (p_sw_rel * srScale)); 
                    } else { 
                ...
                if (synthActive) {
                    for(int i = 0; i < framesRead; i++) {
                        synthEnv = (envBuf[i] > 0.005f) ? __builtin_fminf(1.0f, synthEnv + (p_sy_att * srScale)) : __builtin_fmaxf(0.0f, synthEnv - (p_sy_rel * srScale));
                        ...
                        float fltCoeff = __builtin_fmaxf(0.001f, __builtin_fminf(0.99f, (p_sy_flt + 0.6f * synthEnv) * srScale));
                ...
                if (padActive) {
                    for(int i = 0; i < framesRead; i++) {
                        padEnv = (envBuf[i] > 0.005f) ? __builtin_fminf(1.0f, padEnv + (0.00002f * srScale)) : __builtin_fmaxf(0.0f, padEnv - (0.000005f * srScale));
                        inBuf[i] *= padEnv;
                    }
                }
                ...
                for(int i = 0; i < framesRead; i++) {
                    ...
                    if (!frzActive) { 
                        freezeBuffer[freezeWriteIdxVar] = (int16_t)(__builtin_fmaxf(-1.0f, __builtin_fminf(procSample, 1.0f)) * 32767.0f);
                        freezeWriteIdxVar++; 
                        if (freezeWriteIdxVar >= freezeLength) freezeWriteIdxVar = 0;
                    }
                    
                    if (localFrzRamp > 0.0f || frzActive) {
                        localFrzRamp = frzActive ? __builtin_fminf(1.0f, localFrzRamp + (p_fz_att * srScale)) : __builtin_fmaxf(0.0f, localFrzRamp - (p_fz_rel * srScale)); 
                    }
                    ...
                    delayBuffer[writeIndex] = (int16_t)(__builtin_fmaxf(-1.0f, __builtin_fminf(delayIn, 1.0f)) * 32767.0f);
                    
                    float spd1 = currentPitch;
                    if (vibratoActive) {
                        localVibPhase += globalVibratoPhaseInc; 
                        if (localVibPhase >= LFO_LUT_SIZE) localVibPhase -= LFO_LUT_SIZE;
                        spd1 *= 1.0f + ((lfoLUT[(int)localVibPhase & 1023] - 1.0f) * p_vb_dep);
                    }
                    
                    float spd2 = currentPitch * globalHarmRatio; 
                    float spd3 = currentPitch * globalChorusRatio;
                    if (chorusActive) { 
                        localChoPhase += chorusPhaseIncr; 
                        if (localChoPhase >= LFO_LUT_SIZE) localChoPhase -= LFO_LUT_SIZE;
                        spd3 *= lfoLUT[(int)localChoPhase & 1023]; 
                    }
                    ...
                    if (feedbackActive || localFbRamp > 0.0f) { 
                        ...
                        float lfoVal = lfoLUT[(int)localFbPhase & 1023]; 
                        spd4 = lfoVal; 
                        spd5 = currentPitch * globalFbRatio * lfoVal; 
                        
                        float w4 = processTap(tap_w4_1, delayBuffer, writeIndex, windowMask, hannIntMult) + 
                                   processTap(tap_w4_2, delayBuffer, writeIndex, windowMask, hannIntMult);
                        float w5 = processTap(tap_w5_1, delayBuffer, writeIndex, windowMask, hannIntMult) + 
                                   processTap(tap_w5_2, delayBuffer, writeIndex, windowMask, hannIntMult);
                            
                        if (feedbackActive) {
                            localFbRamp = (envBuf[i] > 0.005f) ? __builtin_fminf(1.0f, localFbRamp + (0.000011f * srScale)) : __builtin_fmaxf(0.0f, localFbRamp - (0.005f * srScale));
                        } else { 
                            localFbRamp = __builtin_fmaxf(0.0f, localFbRamp - (0.0001f * srScale)); 
                        }
                        
                        float mixV = __builtin_fmaxf(0.0f, __builtin_fminf((localFbRamp - 0.1f) * 2.0f, 1.0f));
                        ...
                        float rawDrive = (feedInput - localFbHpf) * p_fb_drv;
                    ...
                    w1Buf[i] = processTap(tap_w1_1, delayBuffer, writeIndex, windowMask, hannIntMult) + 
                               processTap(tap_w1_2, delayBuffer, writeIndex, windowMask, hannIntMult);
                    
                    w2Buf[i] = 0.0f;
                    if (harmActive) {
                        w2Buf[i] = processTap(tap_w2_1, delayBuffer, writeIndex, windowMask, hannIntMult) + 
                                   processTap(tap_w2_2, delayBuffer, writeIndex, windowMask, hannIntMult);
                    }
                    
                    w3Buf[i] = 0.0f;
                    if (chorusActive) {
                        w3Buf[i] = processTap(tap_w3_1, delayBuffer, writeIndex, windowMask, hannIntMult) + 
                                   processTap(tap_w3_2, delayBuffer, writeIndex, windowMask, hannIntMult);
                    }
                    ...
                    if (padActive) { 
                        padFilter = padFilter * pdSmCoeff + w1Buf[i] * (1.0f - pdSmCoeff) + DC_OFFSET; 
                    } else { 
                        padFilter = padFilter * pdSmCoeff + DC_OFFSET; 
                    }
                    
                    int dryIdx = (writeIndex - halfWindow + MAX_BUFFER_SIZE) & BUFFER_MASK;
                    dryBuf[i] = (float)delayBuffer[dryIdx] * 3.0517578125e-5f; 
                    padFilterBuf[i] = padFilter;
                    fbOutBuf[i] = fbOutNode;
                    
                    writeIndex = (writeIndex + 1) & BUFFER_MASK;
                }
                
                vibratoLfoPhase = localVibPhase;
                chorusLfoPhase = localChoPhase;
                feedbackLfoPhase = localFbPhase;
                fbHpfState = localFbHpf;
                ...
                for(int i = 0; i < framesRead; i++) {
                    ...
                    if (!activeGroup && localFrzRamp <= 0.0f && localFbRamp <= 0.0f && padFilterBuf[i] <= 0.001f) {
                        ...
                        sMix += w2Buf[i] * g_w2; 
                        sMix += w3Buf[i] * g_w3;
                        sMix += padFilterBuf[i] * g_pad; 
                        sMix += fzOutBuf[i] * g_frz; 
                        sMix += fbOutBuf[i] * g_fb;
                ...
                for(int i = 0; i < framesRead; i++) {
                    ...
                    float rawOut = sMixBuf[i] * (1.0f / 2147483520.0f);
                ...
                core1_load = core1_load * 0.95f + __builtin_fminf(100.0f, currentLoadPercentage) * 0.05f; 
...
void MidiTask(void * pvParameters) {
    ...
    for (;;) {
        ...
        if (digitalRead(BOOT_SENSE_PIN) == HIGH) {
            ...
            if (moveA || moveB || moveC) {
                ...
                if (moveA) { 
                    ...
                    pedals.updateLastMidiA(); 
                    currentPB1 = calA; 
                }
                
                if (moveB) { 
                    Control_Surface.sendPitchBend(Channel_2, calB); 
                    pedals.updateLastMidiB(); 
                    currentPB2 = calB; 
                }
        ...
        if (pb2ToggleRequested) {
            pb2ToggleRequested = false;
