// v1.7 Lilygo Release Controller (Production / Dynamic Bipolar UI Edition)
...
void goToLightSleep() {
    ...
    // FIX: Swallow the physical release so loop() doesn't falsely trigger a mode swap
...
void DisplayTask(void * pvParameters) {
    ...
    for(;;) {
        ...
        if(!isScreenOff.load(std::memory_order_acquire)) {
            ...
            // Re-draw background foundations strictly when modes switch to prevent tearing
            ...
            // FIX: Space padded formatting overwrites bounding boxes cleanly without fillRect flickers
            ...
            sprintf(buf, "CPU:%3d%%  SRM:%dK  PSR:%dK  SR:%dkHz", 
...
void loop() {
    ...
    // FIX: NVS Cache protection logic implemented
    if(settingsNeedSaving && (millis()-lastParameterChangeTime>2000)) { 
        if (ui_audio_level.load(std::memory_order_acquire) < 0.02f) {
            settingsNeedSaving=false; 
            dsp_is_paused.store(true, std::memory_order_release);
            while(!dsp_ack_parked.load(std::memory_order_acquire)) { vTaskDelay(pdMS_TO_TICKS(1)); }
            saveSettings();
