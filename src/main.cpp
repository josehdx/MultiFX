...
void updateDisplay() {
    ...
    if (isChorusMode && activeEffectMode != 7) {
        spr.setTextColor(TFT_SKYBLUE, TFT_BLACK);
        spr.drawString("* CHORUS *", spr.width() / 2, bgY);
        bgY -= 30;
    }
    ...
    for(;;) {
        ...
        if (bytes_read > 0) {
            ...
            for (int i = 0; i < HOP_SIZE; i++) {
                ...
                if (chorus) {
                    chorusLfoPhase += (1.5f * LFO_LUT_SIZE) / SAMPLING_FREQUENCY;
                ...
                if (!isWhammyActive) {
                    ...
                    shiftedOutput = input + ((w1 + w2) * 0.5f);
                } else if (feedback) {
                    shiftedOutput = input + feedbackOut;
...
bool channelMessageCallback(ChannelMessage cm) {
    if (cm.header == 0xC0 && cm.data1 <= 7) {
        ...
        if (cm.data1 == 5 && cm.data2 == 0) { 
            ...
            if (activeEffectMode == 1) {
                isFrozen = isWhammyActive;
            }
            if (activeEffectMode == 2) {
                isFeedbackActive = isWhammyActive;
            }
            if (activeEffectMode == 3) {
                isHarmonizerMode = isWhammyActive;
            }
            if (activeEffectMode == 4) {
                isCapoMode = isWhammyActive;
            }
            if (activeEffectMode == 7) {
                isChorusMode = isWhammyActive; 
            }
            updateLUT(); 
            forceUIUpdate = true; 
        }
