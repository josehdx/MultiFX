...
void turnScreenOff() { if(!isScreenOff.load(std::memory_order_acquire)) { digitalWrite(38, LOW); digitalWrite(15, LOW); isScreenOff.store(true, std::memory_order_release); } }
void turnScreenOn() { if(isScreenOff.load(std::memory_order_acquire) && !wakeupPending.load(std::memory_order_acquire)) wakeupPending.store(true, std::memory_order_release); }
...
inline float IRAM_ATTR __attribute__((hot)) __attribute__((always_inline)) __attribute__((optimize("O3"))) processTap(uint32_t tapPhase, const int16_t* buffer, int currentWriteIdx, uint32_t windowMask, uint32_t hannIntMult) {
    ...
    // FPU Dual-Issue independent memory loads
    ...
    float& peakInputVal, float& peakOutputVal) 
{
    ...
    #pragma GCC unroll 4
    for(int i = 0; i < framesRead; i++) { 
        // 1. Independent FPU Memory Loads
