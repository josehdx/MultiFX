...
// --- EFFECT STATE (WHAMMY DEFAULT ON, OTHERS OFF) ---
int activeEffectMode = 0; 
float effectMemory[9] = { 12.0f, 12.0f, 12.0f, 5.0f, -2.0f, -12.0f, -12.0f, 12.0f, 0.0f };
...
void updateDisplay() {
    ...
    else if (activeEffectMode == 5) isCurrentEffectActive = isSynthMode;
    else if (activeEffectMode == 6) isCurrentEffectActive = isPadMode;
    else if (activeEffectMode == 7) isCurrentEffectActive = isChorusMode;
    ...
    int rectW = 40;
    int rectH = 16;
    int rectX = outX - rectW - 6;
    int rectY = (spr.height() / 2) - (rectH / 2); 
...
// --- AUDIO DSP TASK (CORRECTED VECTOR OPTIMIZED) ---
    ...
    for(;;) {
        ...
        if (bytes_read > 0) {
            ...
            if (currentWindowSize != targetWin) {
                ...
                tap2 = targetWin / 4.0f; 
                tap1_2 = 0.0f; 
                tap2_2 = targetWin / 4.0f;
            }
            
            // --- THE FIX: FLOATING-POINT DIVISION KILL-SWITCH ---
            ...
            if (synth) {
                for (int i = 0; i < HOP_SIZE; i++) {
                    ...
                    if (freezeRamp > 0.0f) {
                        ...
                        freezeReadIdx++; 
                        if (freezeReadIdx >= freezeLength) freezeReadIdx = 0;
                    }

                    float valToWrite = (freezeActive && freezeRamp > 0.0f) ? freezeOut : writeVal;
                    delayBuffer[writeIndex] = (int16_t)fmaxf(-32768.0f, fminf(valToWrite * 32767.0f, 32767.0f));
                    ...
                    if (chorus) {
                        ...
                        int p2 = (p1 + (LFO_LUT_SIZE / 2)) % LFO_LUT_SIZE;
                    ...
                    tap1 += r1; while (tap1 >= currentWindowSize) tap1 -= currentWindowSize; while (tap1 < 0.0f) tap1 += currentWindowSize;
                    tap2 += r1; while (tap2 >= currentWindowSize) tap2 -= currentWindowSize; while (tap2 < 0.0f) tap2 += currentWindowSize;
                    tap1_2 += r2; while (tap1_2 >= currentWindowSize) tap1_2 -= currentWindowSize; while (tap1_2 < 0.0f) tap1_2 += currentWindowSize;
                    tap2_2 += r2; while (tap2_2 >= currentWindowSize) tap2_2 -= currentWindowSize; while (tap2_2 < 0.0f) tap2_2 += currentWindowSize;
                    ...
                    if (!isWhammyActive) shiftedOutput = input + feedbackOut; 
                    else if (harm) shiftedOutput = input + ((w1 + w2) * 0.707f); 
                    else if (chorus) shiftedOutput = input + ((w1 + w2) * 0.5f);
                    else if (feedback) shiftedOutput = input + feedbackOut;
                    else if (activeEffectMode == 1) shiftedOutput = input + (w1 * currentWetBlend); 
                    else shiftedOutput = (input * (1.0f - currentWetBlend)) + (w1 * currentWetBlend);
    ...
    for(;;) {
        ...
        if (btnCar.update(100)) {
            if (btnCar.state == LOW) {
                ...
                if (dur < 400) { 
                    activeEffectMode = (activeEffectMode + 1) % 8;
...
bool channelMessageCallback(ChannelMessage cm) {
    if (cm.header == 0xB0) {
        ...
        if (cm.data1 == 4 && cm.data2 == 127) { 
            activeEffectMode = (activeEffectMode == 0) ? 7 : (activeEffectMode - 1);
        ...
        else if (cm.data1 == 5 && cm.data2 == 127) { 
            activeEffectMode = (activeEffectMode + 1) % 8;
        ...
        else if (cm.data1 == 6 && cm.data2 == 127) { 
            ...
            isFrozen = (cm.data2 == 127); 
            if (activeEffectMode == 1) isWhammyActive = isFrozen; 
            forceUIUpdate = true; 
        }
        else if (cm.data1 == 9) { 
            isFeedbackActive = (cm.data2 == 127); 
            if (activeEffectMode == 2) isWhammyActive = isFeedbackActive; 
            forceUIUpdate = true; 
        }
        else if (cm.data1 == 10) { 
            isHarmonizerMode = (cm.data2 == 127); 
            if (activeEffectMode == 3) isWhammyActive = isHarmonizerMode; 
            forceUIUpdate = true; 
        }
        else if (cm.data1 == 12) { 
            isCapoMode = (cm.data2 == 127); 
            if (activeEffectMode == 4) isWhammyActive = isCapoMode; 
            updateLUT(); 
            forceUIUpdate = true; 
        }
        else if (cm.data1 == 13) { 
            isSynthMode = (cm.data2 == 127); 
            if (activeEffectMode == 5) isWhammyActive = isSynthMode; 
            forceUIUpdate = true; 
        }
        else if (cm.data1 == 14) { 
            isPadMode = (cm.data2 == 127); 
            if (activeEffectMode == 6) isWhammyActive = isPadMode; 
            forceUIUpdate = true; 
        }
        else if (cm.data1 == 15) { 
            isChorusMode = (cm.data2 == 127); 
            if (activeEffectMode == 7) isWhammyActive = isChorusMode; 
            forceUIUpdate = true; 
        }
        else if (cm.data1 == 17) {
            if (activeEffectMode == 0) {
                if (cm.data2 == 0) effectMemory[5] = constrain(effectMemory[5] + 1.0f, -24.0f, 24.0f);
                else if (cm.data2 == 127) effectMemory[5] = constrain(effectMemory[5] - 1.0f, -24.0f, 24.0f);
            } else {
                if (cm.data2 == 127) {
            ...
            if (activeEffectMode == 0) {
                if (cm.data2 == 0) effectMemory[0] = constrain(effectMemory[0] + 1.0f, -24.0f, 24.0f);
                else if (cm.data2 == 127) effectMemory[0] = constrain(effectMemory[0] - 1.0f, -24.0f, 24.0f);
            } else {
                if (cm.data2 == 0) {
                    int slot = activeEffectMode; 
                    if (activeEffectMode == 5) slot = 6; 
