// v1.22 Lilygo Release Controller (UI Separation, Circular Sliders & Telemetry)
...
void saveSettings() {
    ...
    if (firstSave || modeCopy != lastSavedMode) { preferences.putInt("activeMode", modeCopy); lastSavedMode = modeCopy; }
    if (firstSave || latCopy != lastSavedLatMode) { preferences.putInt("latMode", latCopy); lastSavedLatMode = latCopy; }
    if (firstSave || pb2Copy != lastSavedPb2Wiper) { preferences.putBool("pb2Wiper", pb2Copy); lastSavedPb2Wiper = pb2Copy; }
    if (firstSave || volCopy != lastSavedVolMode) { preferences.putBool("volMode", volCopy); lastSavedVolMode = volCopy; }
    if (firstSave || fxStates != lastSavedFxStates) { preferences.putUShort("fxStates", fxStates); lastSavedFxStates = fxStates; }
    if (firstSave || srCopy != lastSavedSampleRate) { preferences.putUInt("sampleRate", srCopy); lastSavedSampleRate = srCopy; }
    if (firstSave || fbCopy != lastSavedFbIdx) { preferences.putInt("fbIdx", fbCopy); lastSavedFbIdx = fbCopy; }
    
    bool dspDataChanged = firstSave;
    if (!dspDataChanged) {
        if (memcmp(&cs, &lastSavedDspData, sizeof(AppSettings)) != 0) dspDataChanged = true;
    }
    if (dspDataChanged) {
