...
std::atomic<float> core1_load{0.0f};
...
std::atomic<float> ui_audio_level{0.0f};
std::atomic<float> ui_output_level{0.0f};
...
// Hardware FPU Flush-To-Zero
inline void IRAM_ATTR enable_fpu_ftz() {
    uint32_t fcr;
    __asm__ __volatile__ ("rsr.fcr %0" : "=r" (fcr));
    fcr |= (1 << 0);
    __asm__ __volatile__ ("wur.fcr %0" : : "r" (fcr));
}
...
void IRAM_ATTR __attribute__((optimize("O3"))) updateLUT() {
    ...
    if(isCapoMode || (activeEffectMode.load(std::memory_order_acquire)==4 && isWhammyActive)) basePitch+=effectMemory[4];
    float toeBend=effectMemory[0], heelBend=effectMemory[1], harmRatioMem=effectMemory[3], chorusRatioMem=effectMemory[7], vibHzMem=effectMemory[9]; 
    int fbIntervalIdxLocal=feedbackIntervalIdx.load(std::memory_order_acquire);
    if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex);
    ...
    float fbIntervals[5]={0.0f,12.0f,19.0f,24.0f,28.0f}; globalFbRatio=powf(2.0f, fbIntervals[constrain(fbIntervalIdxLocal,0,4)]/12.0f);
...
void updateDisplay() {
    ...
    if(renderMode==0||renderMode==1||renderMode==8) { 
        bGauges[numG++]={1,effectMemory[1]/24.0f,"HEEL",""}; snprintf(bGauges[numG-1].valStr,16,"%+.1f",effectMemory[1]); 
        bGauges[numG++]={1,effectMemory[0]/24.0f,"TOE",""}; snprintf(bGauges[numG-1].valStr,16,"%+.1f",effectMemory[0]); 
    }
    else if(renderMode==2) { 
        float fbi[]={0.0f,12.0f,19.0f,24.0f,28.0f}; float val=fbi[constrain((int)feedbackIntervalIdx.load(std::memory_order_acquire),0,4)]; 
    ...
    if(renderMode==0) { 
        bGauges[numG++]={2,fxParams[0][0],"D.MIX",""}; snprintf(bGauges[numG-1].valStr,16,"%.1f",fxParams[0][0]); 
        bGauges[numG++]={2,fxParams[0][1],"W.MIX",""}; snprintf(bGauges[numG-1].valStr,16,"%.1f",fxParams[0][1]); 
    }
    else if(renderMode==1) { 
        bGauges[numG++]={2,fxParams[1][0]/0.95f,"RVB",""}; snprintf(bGauges[numG-1].valStr,16,"%.2f",fxParams[1][0]); 
        bGauges[numG++]={2,(fxParams[1][1]-0.00001f)/0.001f,"ATK",""}; snprintf(bGauges[numG-1].valStr,16,"%.2f",fxParams[1][1]*1000.0f); 
        bGauges[numG++]={2,(fxParams[1][2]-0.00001f)/0.0005f,"REL",""}; snprintf(bGauges[numG-1].valStr,16,"%.2f",fxParams[1][2]*1000.0f); 
    }
    else if(renderMode==2) { 
        bGauges[numG++]={2,(fxParams[2][0]-1000.0f)/10000.0f,"SPD",""}; snprintf(bGauges[numG-1].valStr,16,"%.0f",fxParams[2][0]); 
        bGauges[numG++]={2,(fxParams[2][1]-1.0f)/100.0f,"DRV",""}; snprintf(bGauges[numG-1].valStr,16,"%.1f",fxParams[2][1]); 
        bGauges[numG++]={2,(fxParams[2][2]-0.005f)/0.045f,"DLY",""}; snprintf(bGauges[numG-1].valStr,16,"%.2f",fxParams[2][2]); 
    }
    else if(renderMode==3) { bGauges[numG++]={2,fxParams[3][0],"MIX",""}; snprintf(bGauges[numG-1].valStr,16,"%.2f",fxParams[3][0]); }
    else if(renderMode==5) { 
        bGauges[numG++]={2,(fxParams[5][0]-0.01f)/0.5f,"ATK",""}; snprintf(bGauges[numG-1].valStr,16,"%.2f",fxParams[5][0]); 
        bGauges[numG++]={2,(fxParams[5][1]-0.001f)/0.05f,"REL",""}; snprintf(bGauges[numG-1].valStr,16,"%.2f",fxParams[5][1]); 
        bGauges[numG++]={2,(fxParams[5][2]-0.1f)/0.8f,"FLT",""}; snprintf(bGauges[numG-1].valStr,16,"%.2f",fxParams[5][2]); 
        bGauges[numG++]={2,fxParams[5][3],"MIX",""}; snprintf(bGauges[numG-1].valStr,16,"%.2f",fxParams[5][3]); 
    }
    else if(renderMode==6) { 
        bGauges[numG++]={2,(fxParams[6][0]-0.8f)/0.199f,"TON",""}; snprintf(bGauges[numG-1].valStr,16,"%.2f",fxParams[6][0]); 
        bGauges[numG++]={2,fxParams[6][1]/3.0f,"MIX",""}; snprintf(bGauges[numG-1].valStr,16,"%.1f",fxParams[6][1]); 
    }
    else if(renderMode==7) { 
        bGauges[numG++]={2,(fxParams[7][0]-500.0f)/4500.0f,"SPD",""}; snprintf(bGauges[numG-1].valStr,16,"%.0f",fxParams[7][0]); 
        bGauges[numG++]={2,fxParams[7][1],"MIX",""}; snprintf(bGauges[numG-1].valStr,16,"%.2f",fxParams[7][1]); 
    }
    else if(renderMode==8) { 
        bGauges[numG++]={2,(fxParams[8][0]-0.001f)/0.05f,"THR",""}; snprintf(bGauges[numG-1].valStr,16,"%.2f",fxParams[8][0]); 
        bGauges[numG++]={2,(fxParams[8][1]-0.00001f)/0.0005f,"ATK",""}; snprintf(bGauges[numG-1].valStr,16,"%.2f",fxParams[8][1]*1000.0f); 
        bGauges[numG++]={2,(fxParams[8][2]-0.00001f)/0.0005f,"REL",""}; snprintf(bGauges[numG-1].valStr,16,"%.2f",fxParams[8][2]*1000.0f); 
    }
    else if(renderMode==9) { bGauges[numG++]={2,fxParams[9][0]/2.0f,"DEP",""}; snprintf(bGauges[numG-1].valStr,16,"%+.1f",fxParams[9][0]); }
    ...
    const char* latencyLabelStrings[]={"U.Low","Low","Mid","High"}; spr.drawString(latencyLabelStrings[latencyMode.load(std::memory_order_acquire)],275,statsRowY);
...
void IRAM_ATTR __attribute__((optimize("O3"))) DSP_ProcessInput(
    ...
    for(int i = 0; i < framesRead; i++) {
        ...
        localDC = __builtin_fmaf(raw_in, dc_alpha, localDC * (1.0f - dc_alpha));
        float inSample = raw_in - localDC;
        
        localEnv = __builtin_fmaf(fabsf(inSample), envAttack, __builtin_fmaf(localEnv, envRetain, 1e-9f));
            ...
            if(__builtin_expect(framesRead>0, 1)) {
                ...
                if(__builtin_expect(currentMute > 0, 0)) {
                     ...
                     for(int i=0; i<framesRead; i++) {
                         ...
                         float raw_in=((float)clean_sample*normFactor);
                         input_dc_offset=__builtin_fmaf(raw_in, 0.05f, input_dc_offset * 0.95f);
                     }
                ...
                for(int i=0; i<framesRead; i++) {
                    ...
                    if(__builtin_expect(feedbackActive||localFbRamp>0.0f, 0)) {
                        ...
                        if(feedbackActive) localFbRamp=(envBuf[i]>0.005f)?__builtin_fminf(1.0f,__builtin_fmaf(0.000011f, srScale, localFbRamp)):__builtin_fmaxf(0.0f,__builtin_fmaf(-0.005f, srScale, localFbRamp)); else localFbRamp=__builtin_fmaxf(0.0f,__builtin_fmaf(-0.0001f, srScale, localFbRamp));
                    ...
                    if(__builtin_expect(padActive, 0)) padFilter=__builtin_fmaf(padFilter, pdSmCoeff, __builtin_fmaf(w1Buf[i], (1.0f-pdSmCoeff), DC_OFFSET)); else padFilter=__builtin_fmaf(padFilter, pdSmCoeff, DC_OFFSET); 
...
void setup() {
    ...
    xTaskCreatePinnedToCore(MidiTask, "Midi", 8192, NULL, configMAX_PRIORITIES-1, NULL, 0);
