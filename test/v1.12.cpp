// v1.12 Lilygo Release Controller (Studio Enhancements Edition)
...
void IRAM_ATTR __attribute__((optimize("O3"))) AudioDSPTask(void * pvParameters) {
    ...
    float dampState = 0.0f; // DYNAMIC DAMPING FILTER STATE
        ...
        if(__builtin_expect(bytesRead>0, 1)) {
            ...
            if(__builtin_expect(framesRead == HOP_SIZE, 1)) {
                if(__builtin_expect(panicResetRequested.load(std::memory_order_acquire), 0)) {
                    ...
                    synthEnv=0.0f; synthFilter=0.0f; padFilter=0.0f; padEnv=0.0f; inputEnvelope=0.0f; feedbackFilterVar=0.0f; currentPitch=1.0f; freezeWriteIdxVar=0; freezePlayCounterVar=0; freezeStartIdxVar=0; activeFreezeLength=currentSampleRate.load(std::memory_order_acquire); fbDelayWriteIdx=0; apfNeedsClear=true; freezeRamp=0.0f; feedbackRamp=0.0f; vibratoLfoPhase=0.0f; chorusLfoPhase=0.0f; feedbackLfoPhase=0.0f; dampState=0.0f; diffuserIdx=0; if(diffuserBuf) memset(diffuserBuf, 0, 256*sizeof(float));
                ...
                if(__builtin_expect(globalAudioResetRequested.load(std::memory_order_acquire), 0)) {
                    ...
                    synthEnv=0.0f; synthFilter=0.0f; padFilter=0.0f; padEnv=0.0f; inputEnvelope=0.0f; feedbackFilterVar=0.0f; smoothedVolGain=volumePedalGain; currentPitch=1.0f; freezeWriteIdxVar=0; freezePlayCounterVar=0; freezeStartIdxVar=0; activeFreezeLength=currentSampleRate.load(std::memory_order_acquire); fbDelayWriteIdx=0; writeIndex=0; sramWriteIdx=0; apfNeedsClear=true; input_dc_offset=0.0f; ui_audio_level.store(0.0f, std::memory_order_release); ui_output_level.store(0.0f, std::memory_order_release); freezeRamp=0.0f; feedbackRamp=0.0f; vibratoLfoPhase=0.0f; chorusLfoPhase=0.0f; feedbackLfoPhase=0.0f; dampState=0.0f; diffuserIdx=0; if(diffuserBuf) memset(diffuserBuf, 0, 256*sizeof(float));
                    uint32_t halfWinFixed=((uint32_t)currentWindowSize/2)<<16; tap_w1_1=0; tap_w1_2=halfWinFixed; tap_w2_1=0; tap_w2_2=halfWinFixed; tap_w3_1=0; tap_w3_2=halfWinFixed; tap_w4_1=0; tap_w4_2=halfWinFixed; tap_w5_1=0; tap_w5_2=halfWinFixed;
                ...
                for(int i=0; i<framesRead; i++) {
                    ...
                    // --- 3. ALL-PASS 3D DIFFUSION (CHORUS/PAD) ---
