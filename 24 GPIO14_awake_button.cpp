...
// The Master Bypass for whatever mode is currently on screen
volatile bool isWhammyActive = true; 

// Global independent effect flags
...
inline float IRAM_ATTR getHermiteSample(float tapPos, float* buffer, int writeIdx) {
    ...
    float y2 = buffer[idx1];
    float y3 = buffer[idx2];
        ...
        if (bytes_read > 0) {
            ...
            for (int i = 0; i < HOP_SIZE; i++) {
                ...
                if (freezeRamp > 0.0f) {
                    ...
                    int i2 = (freezeReadIdx + freezeLength / 2) % freezeLength;
                ...
                delayBuffer[writeIndex] = (freezeActive && freezeRamp > 0.0f) ? freezeOut : writeVal;
                ...
                if (feedback) {
                    ...
                    if (feedbackLfoPhase > TWO_PI) {
                        feedbackLfoPhase -= TWO_PI;
                    }
                ...
                tap1 += r1;
                while (tap1 >= currentWindowSize) {
                    tap1 -= currentWindowSize;
                }
                while (tap1 < 0.0f) {
                    tap1 += currentWindowSize;
                }
                ...
                tap2 += r1;
                while (tap2 >= currentWindowSize) {
                    tap2 -= currentWindowSize;
                }
                while (tap2 < 0.0f) {
                    tap2 += currentWindowSize;
                }
                ...
                tap1_2 += r2;
                while (tap1_2 >= currentWindowSize) {
                    tap1_2 -= currentWindowSize;
                }
                while (tap1_2 < 0.0f) {
                    tap1_2 += currentWindowSize;
                }
                ...
                tap2_2 += r2;
                while (tap2_2 >= currentWindowSize) {
                    tap2_2 -= currentWindowSize;
                }
                while (tap2_2 < 0.0f) {
                    tap2_2 += currentWindowSize;
                }
                ...
                if (feedback || feedbackRamp > 0.0f) {
                    ...
                    fbDelayWriteIdx = (fbDelayWriteIdx + 1) % 8192;
...
void MidiTask(void * pvParameters) {
    ...
    for(;;) {
        ...
        // --- EXPRESSION PEDAL MATH (ONLY source of Screen timer resets) ---
        filterPB.update(); 
        filterPB2.update(); 
...
void setup() {
    ...
    memset(delayBuffer, 0, MAX_BUFFER_SIZE * 4); 
