...
void __attribute__((optimize("O3"))) updateLUT(); 
void DisplayTask(void * pvParameters); 
void MidiTask(void * pvParameters); 
void __attribute__((optimize("O3"))) AudioDSPTask(void * pvParameters); 
...
DRAM_ATTR float hannLUT[HANN_LUT_SIZE] __attribute__((aligned(64))); DRAM_ATTR float lfoLUT[LFO_LUT_SIZE] __attribute__((aligned(64))); DRAM_ATTR float synthLUT[WAVE_LUT_SIZE] __attribute__((aligned(64)));
...
float apf1Buffer[1009] = {0.0f}, apf2Buffer[863] = {0.0f}; int apf1Idx = 0, apf2Idx = 0; 
std::atomic<int> feedbackIntervalIdx{0}; 
volatile bool lutNeedsUpdate = false; volatile uint16_t lastActivePedal = 8192;
...
std::atomic<int> latencyMode{0}; std::atomic<int> activeEffectMode{0}; std::atomic<int> hardwareSyncMuteFrames{0}; 

volatile bool sampleRateToggleRequested=false, pb2ToggleRequested=false;
unsigned long lastActivityTime=0, lastScreenActivityTime=0; const unsigned long LIGHT_SLEEP_TIMEOUT=25000, SCREEN_OFF_TIMEOUT=15000;
volatile bool sleepRequested=false, isSleeping=false; volatile float core1_load=0.0f; volatile bool isAdcPaused=false;
...
uint16_t lastMidiSent=8192; volatile uint16_t currentPB1=8192, currentPB2=8192, currentPB3=8192, currentCC11=0; 
std::atomic<float> ui_audio_level{0.0f}; std::atomic<float> ui_output_level{0.0f};
...
void fetchADCDMA() {
    if(isAdcPaused) return; uint8_t result[128] __attribute__((aligned(4))); uint32_t ret_num=0; esp_err_t err; int loop_bound = 0;
    while(loop_bound++ < 4) {
        ...
        else { adc_continuous_stop(multifx_adc_handle); adc_continuous_start(multifx_adc_handle); break; }
    }
}
...
void saveSettings() {
    ...
    int modeCopy = activeEffectMode.load(std::memory_order_acquire), latCopy = latencyMode.load(std::memory_order_acquire), fbCopy = constrain((int)feedbackIntervalIdx.load(std::memory_order_acquire),0,4);
    bool pb2Copy = isPB2WiperMode, volCopy = isVolumeMode; uint32_t srCopy = currentSampleRate.load(std::memory_order_acquire);
    if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex);
    preferences.begin("whammy_cfg", false); preferences.putInt("activeMode", modeCopy); preferences.putInt("latMode", latCopy); preferences.putBool("pb2Wiper", pb2Copy); preferences.putBool("volMode", volCopy); preferences.putUShort("fxStates", fxStates); preferences.putUInt("sampleRate", srCopy); preferences.putInt("fbIdx", fbCopy); preferences.putBytes("dspData", &cs, sizeof(AppSettings)); preferences.end();
}
...
void toggleSampleRate() {
    ...
    i2s_channel_disable(tx_chan); i2s_channel_disable(rx_chan); i2s_del_channel(tx_chan); i2s_del_channel(rx_chan);
    ...
    i2s_std_config_t stdConfig={ .clk_cfg=I2S_STD_CLK_DEFAULT_CONFIG(currentSampleRate.load(std::memory_order_acquire)), .slot_cfg=I2S_STD_PHILIPS_SLOT_DEFAULT_CONFIG(I2S_DATA_BIT_WIDTH_32BIT, I2S_SLOT_MODE_STEREO), .gpio_cfg={ .mclk=GPIO_NUM_43, .bclk=GPIO_NUM_44, .ws=GPIO_NUM_18, .dout=GPIO_NUM_16, .din=GPIO_NUM_17 } };
    ...
    memset(delayBuffer, 0, MAX_BUFFER_SIZE * sizeof(int16_t)); memset(fbDelayBuffer, 0, FB_BUFFER_SIZE * sizeof(int16_t)); memset(freezeBuffer, 0, FREEZE_BUFFER_SIZE * sizeof(int16_t)); memset(i2s_in_block, 0, HOP_SIZE * 2 * sizeof(int32_t)); memset(i2s_out_block, 0, HOP_SIZE * 2 * sizeof(int32_t));
    ...
    while(dsp_ack_parked.load(std::memory_order_acquire)) { vTaskDelay(pdMS_TO_TICKS(1)); }
    pedals.triggerSystemRecovery(); sleepRequested = false; forceUIUpdate.store(true, std::memory_order_release); lastParameterChangeTime = millis();
}

void turnScreenOff() { if(!isScreenOff.load(std::memory_order_acquire)) { tft.writecommand(0x10); digitalWrite(38, LOW); digitalWrite(15, LOW); isScreenOff.store(true, std::memory_order_release); } }
void turnScreenOn() { if(isScreenOff.load(std::memory_order_acquire) && !wakeupPending.load(std::memory_order_acquire)) { tft.writecommand(0x11); wakeupPending.store(true, std::memory_order_release); } }
...
void goToLightSleep() {
    ...
    while (digitalRead(BOOT_SENSE_PIN) == HIGH) { 
        uint8_t trash[128] __attribute__((aligned(4))) = {0}; uint32_t ret_num = 0;
    ...
    i2s_channel_enable(tx_chan); i2s_channel_enable(rx_chan); globalAudioResetRequested.store(true, std::memory_order_release);
    ...
    while (isSleeping && timeoutCounter < 10) { vTaskDelay(pdMS_TO_TICKS(10)); timeoutCounter++; }
    vTaskDelay(pdMS_TO_TICKS(200)); if (isScreenOff.load(std::memory_order_acquire)) turnScreenOn(); 
    pedals.triggerSystemRecovery(); lastActivityTime = millis(); lastScreenActivityTime = millis();
}

void __attribute__((optimize("O3"))) updateLUT() {
    if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY);
    float basePitch=0.0f; if(isCapoMode || (activeEffectMode.load(std::memory_order_acquire)==4 && isWhammyActive)) basePitch+=effectMemory[4];
    float toeBend=effectMemory[0], heelBend=effectMemory[1], harmRatioMem=effectMemory[3], chorusRatioMem=effectMemory[7], vibHzMem=effectMemory[9]; int fbIntervalIdxLocal=feedbackIntervalIdx.load(std::memory_order_acquire);
    if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex);
    ...
    for(int i=0; i<16384; i++) {
        ...
        pitchShiftLUT_temp[i]=powf(2.0f,(basePitch+dynamicBend)/12.0f); if(i == 8192) vTaskDelay(pdMS_TO_TICKS(1));
    }
    ...
    float vibHz=(vibHzMem!=0.0f)?fabsf(vibHzMem):2.0f; globalVibratoPhaseInc=(vibHz*LFO_LUT_SIZE)/(float)currentSampleRate.load(std::memory_order_acquire);
}
...
void updateMeters() {
    ...
    int barHeight=98, inFillHeight=constrain((int)(current_ui_in*barHeight),0,barHeight); meterSpr.fillSprite(TFT_BLACK); meterSpr.fillRect(0, barHeight-inFillHeight, 6, inFillHeight, (current_ui_in>0.90f)?TFT_RED:TFT_GREEN); meterSpr.pushSprite(11,31);
    int outFillHeight=constrain((int)(current_ui_out*barHeight),0,barHeight); meterSpr.fillSprite(TFT_BLACK); meterSpr.fillRect(0, barHeight-outFillHeight, 6, outFillHeight, (current_ui_out>0.90f)?TFT_RED:TFT_GREEN); meterSpr.pushSprite(spr.width()-17, 31);
}
...
void updateDisplay() {
    spr.fillSprite(TFT_BLACK); int renderMode=activeEffectMode.load(std::memory_order_acquire); char batStr[24];
    ...
    else if(renderMode==2) { float fbi[]={0.0f,12.0f,19.0f,24.0f,28.0f}; float val=fbi[constrain((int)feedbackIntervalIdx.load(std::memory_order_acquire),0,4)]; bGauges[numG++]={2,val/28.0f,"OVT",""}; snprintf(bGauges[numG-1].valStr,16,"%+.1f",val); }
    ...
    spr.setTextDatum(MC_DATUM); spr.setTextColor(TFT_WHITE); spr.drawRect(210,statsRowY-7,40,14,TFT_DARKGREY); spr.drawString((currentSampleRate.load(std::memory_order_acquire)==96000)?"96k":"48k",230,statsRowY); spr.drawRect(255,statsRowY-7,40,14,TFT_DARKGREY); const char* latencyLabelStrings[]={"U.Low","Low","Mid","High"}; spr.drawString(latencyLabelStrings[latencyMode.load(std::memory_order_acquire)],275,statsRowY);
...
void DisplayTask(void * pvParameters) {
    ...
    for(;;) {
        if(__builtin_expect(wakeupPending.load(std::memory_order_acquire), 0)) { 
            ...
            vTaskDelay(pdMS_TO_TICKS(150)); tft.init(); tft.setRotation(1); 
        ...
        if(__builtin_expect(forceUIUpdate.exchange(false, std::memory_order_acq_rel) && !isScreenOff.load(std::memory_order_acquire), 1)) { 
            updateDisplay(); metersNeedClear=true; 
        }
        else if(__builtin_expect(!isScreenOff.load(std::memory_order_acquire), 1)) { 
...
inline float IRAM_ATTR __attribute__((hot)) __attribute__((always_inline)) __attribute__((optimize("O3"))) processTap(uint32_t tapPhase, const int16_t* buffer, int currentWriteIdx, uint32_t windowMask, uint32_t hannIntMult) {
    ...
    float d3 = y3 - y0;
    float c1 = 0.5f * d1; 
    float c3 = 1.5f * d2 + 0.5f * d3; 
    float c2 = y0 - y1 + c1 - c3; 
    float sample = ((c3 * frac + c2) * frac + c1) * frac + y1; 
    ...
    float targetPitch, float* envBuf, float* masterGainBuf, float* inBuf, float* fzOutBuf) 
{
    ...
    for(int i = 0; i < framesRead; i++) {
        ...
        localDC = AntiDenormal((localDC * (1.0f - dc_alpha)) + (raw_in * dc_alpha));
        float inSample = raw_in - localDC; 
        
        localEnv = localEnv * envRetain + fabsf(inSample) * envAttack + 1e-9f; 
        
        if(__builtin_expect(swellActive, 0)) {
            localSwell = (localEnv > p_sw_thr) ? 
                __builtin_fminf(1.0f, localSwell + (p_sw_att * srScale)) : 
                __builtin_fmaxf(0.0f, localSwell - (p_sw_rel * srScale)); 
        } else {
            localSwell = __builtin_fminf(1.0f, localSwell + (0.005f * srScale));
        }
        
        localSmVol = AntiDenormal(localSmVol * (1.0f - vol_alpha) + localVolGain * vol_alpha + 1e-9f); 
        float mGain = 2147483520.0f * localSwell * localSmVol; 
    ...
    float& peakInputVal, float& peakOutputVal) 
{
    ...
    for(int i = 0; i < framesRead; i++) { 
        ...
        if(__builtin_expect(!activeGroup && localFrzRamp <= 0.0f && localFbRamp <= 0.0f && b_pad <= 0.001f, 0)) { 
            ...
            float baseSignal = (b_w1 * g_whammy) + (b_dry * g_dry); 
    ...
    for(int i = 0; i < framesRead; i++) { 
        ...
        float rawOut = b_mix * (1.0f / 2147483520.0f); 
        float abs_out = fabsf(rawOut);
        ...
        int32_t finalOut = (int32_t)__builtin_fmaxf(-2147483520.0f, __builtin_fminf(b_mix, 2147483520.0f)); 
        finalOut &= 0xFFFFFF00; 
...
void AudioDSPTask(void * pvParameters) {
    ...
    bool wasSleeping=false; float c_fx[10][5] = {0.0f}; int c_lat=0, c_act=0;
    ...
    for(;;) {
        ...
        if(__builtin_expect(sleepRequested, 0)) { isSleeping=true; wasSleeping=true; vTaskDelay(pdMS_TO_TICKS(10)); continue; }
        isSleeping=false;
        if(__builtin_expect(wasSleeping, 0)) { size_t dummyBytes; int32_t flushBuf[HOP_SIZE*2]; ulTaskNotifyTake(pdTRUE, 0); while(i2s_channel_read(rx_chan,flushBuf,sizeof(flushBuf),&dummyBytes,0)==ESP_OK && dummyBytes>0) {} wasSleeping=false; }
        
        ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(20)); size_t bytesRead; i2s_channel_read(rx_chan, i2s_in_block, HOP_SIZE*2*sizeof(int32_t), &bytesRead, pdMS_TO_TICKS(10));
            ...
            if(__builtin_expect(framesRead>0, 1)) {
                ...
                if(__builtin_expect(currentMute > 0, 0)) { 
                    ...
                    for(int i=0; i<framesRead; i++) { 
                        ...
                        input_dc_offset = AntiDenormal(input_dc_offset); 
                    } 
                ...
                if (audioBufferMutex != NULL && xSemaphoreTake(audioBufferMutex, 0) == pdTRUE) {
                    ...
                    c_lat=latencyMode.load(std::memory_order_acquire); 
                    c_act=activeEffectMode.load(std::memory_order_acquire); 
                    c_w=isWhammyActive; c_fz=isFrozen; 
                    c_fb=isFeedbackActive; c_hr=isHarmonizerMode; c_cp=isCapoMode; c_sy=isSynthMode; 
                    c_pd=isPadMode; c_ch=isChorusMode; c_sw=isSwellMode; c_vb=isVibratoMode; 
                ...
                if(__builtin_expect(isnan(synthFilter)||isinf(synthFilter), 0)) synthFilter=0.0f; if(__builtin_expect(isnan(padFilter)||isinf(padFilter), 0)) padFilter=0.0f; if(__builtin_expect(isnan(feedbackFilterVar)||isinf(feedbackFilterVar), 0)) feedbackFilterVar=0.0f; if(__builtin_expect(isnan(fbHpfState)||isinf(fbHpfState), 0)) fbHpfState=0.0f; float localVibPhase=vibratoLfoPhase, localChoPhase=chorusLfoPhase, localFbPhase=feedbackLfoPhase, localFbHpf=fbHpfState; int prefetchIdxW1=(writeIndex-halfWindow+MAX_BUFFER_SIZE)&BUFFER_MASK, prefetchIdxFB=(fbDelayWriteIdx-delaySamples+FB_BUFFER_SIZE)&FB_BUFFER_MASK; __builtin_prefetch(&delayBuffer[prefetchIdxW1], 0, 3); __builtin_prefetch(&fbDelayBuffer[prefetchIdxFB], 0, 3);
                ...
                for(int i=0; i<framesRead; i++) {
                    ...
                    if(__builtin_expect(localFrzRamp>0.0f||frzActive, 0)) localFrzRamp=frzActive?__builtin_fminf(1.0f,localFrzRamp+(p_fz_att*srScale)):__builtin_fmaxf(0.0f,localFrzRamp-(p_fz_rel*srScale));
                    if(__builtin_expect(localFrzRamp>0.0f, 0)) {
                        ...
                        float d1=apf1Buffer[apf1Idx], next_apf1=rFrz+p_fz_apf*d1+DC_OFFSET, a1=-p_fz_apf*rFrz+d1; apf1Buffer[apf1Idx]=AntiDenormal(next_apf1); apf1Idx++; if(apf1Idx>=1009) apf1Idx=0; float d2=apf2Buffer[apf2Idx], next_apf2=a1+p_fz_apf*d2+DC_OFFSET, a2=-p_fz_apf*a1+d2; apf2Buffer[apf2Idx]=AntiDenormal(next_apf2); apf2Idx++; if(apf2Idx>=863) apf2Idx=0; fzOutBuf[i]=a2*localFrzRamp; freezePlayCounterVar++; if(freezePlayCounterVar>=activeFreezeLength) freezePlayCounterVar=0;
                    ...
                    float spd1=currentPitch; if(__builtin_expect(vibratoActive, 0)) { localVibPhase+=globalVibratoPhaseInc; if(localVibPhase>=LFO_LUT_SIZE) localVibPhase-=LFO_LUT_SIZE; spd1*=1.0f+((lfoLUT[(int)localVibPhase&1023]-1.0f)*p_vb_dep); }
                    float spd2=currentPitch*globalHarmRatio, spd3=currentPitch*globalChorusRatio; if(__builtin_expect(chorusActive, 0)) { localChoPhase+=chorusPhaseIncr; if(localChoPhase>=LFO_LUT_SIZE) localChoPhase-=LFO_LUT_SIZE; spd3*=lfoLUT[(int)localChoPhase&1023]; } float spd4=1.0f, spd5=1.0f;
                    ...
                    if(__builtin_expect(feedbackActive||localFbRamp>0.0f, 0)) {
                        ...
                        localFbHpf=AntiDenormal(localFbHpf); 
                        float rawDrive=(feedInput-localFbHpf)*p_fb_drv, boundedDrive=__builtin_fmaxf(-1.5f,__builtin_fminf(rawDrive,1.5f)), gainDrive=boundedDrive*(1.0f-(0.15f*boundedDrive*boundedDrive)); feedbackFilterVar=feedbackFilterVar*fbLpfRetain+gainDrive*fbLpfCoeff+DC_OFFSET; feedbackFilterVar=AntiDenormal(feedbackFilterVar); float satFb=feedbackFilterVar*(localFbRamp*localFbRamp*localFbRamp)*0.85f; fbDelayBuffer[fbDelayWriteIdx]=(int16_t)(__builtin_fmaxf(-1.0f,__builtin_fminf(satFb,1.0f))*32767.0f);
                    ...
                    if(__builtin_expect(padActive, 0)) padFilter=padFilter*pdSmCoeff+w1Buf[i]*(1.0f-pdSmCoeff)+DC_OFFSET; else padFilter=padFilter*pdSmCoeff+DC_OFFSET; padFilter=AntiDenormal(padFilter); int dryIdx=(writeIndex-halfWindow+MAX_BUFFER_SIZE)&BUFFER_MASK; dryBuf[i]=(float)delayBuffer[dryIdx]*3.0517578125e-5f; padFilterBuf[i]=padFilter; fbOutBuf[i]=fbOutNode; writeIndex=(writeIndex+1)&BUFFER_MASK;
                }
...
void updateParameterFromCC(uint8_t cc, uint8_t val) {
    ...
    if(audioBufferMutex!=NULL && xSemaphoreTake(audioBufferMutex, 0) == pdTRUE) {
        ...
        if(currentMode==0) { if(pIdx==0) { effectMemory[1]=roundf((norm*48.0f)-24.0f); lutNeedsUpdate=true; forceUIUpdate.store(true, std::memory_order_release); } if(pIdx==1) { effectMemory[0]=roundf((norm*48.0f)-24.0f); lutNeedsUpdate=true; forceUIUpdate.store(true, std::memory_order_release); } if(pIdx==2) fxParams[0][0]=norm; if(pIdx==3) fxParams[0][1]=norm; }
        else if(currentMode==1) { if(pIdx==0) { effectMemory[1]=roundf((norm*48.0f)-24.0f); lutNeedsUpdate=true; forceUIUpdate.store(true, std::memory_order_release); } if(pIdx==1) { effectMemory[0]=roundf((norm*48.0f)-24.0f); lutNeedsUpdate=true; forceUIUpdate.store(true, std::memory_order_release); } if(pIdx==2) fxParams[1][0]=0.0f+(norm*0.95f); if(pIdx==3) fxParams[1][1]=0.00001f+(norm*0.001f); if(pIdx==4) fxParams[1][2]=0.00001f+(norm*0.0005f); }
        else if(currentMode==2) { if(pIdx==0) { int newIdx=constrain((int)roundf(norm*4.0f),0,4); if(newIdx!=feedbackIntervalIdx.load(std::memory_order_acquire)) { feedbackIntervalIdx.store(newIdx, std::memory_order_release); lutNeedsUpdate=true; forceUIUpdate.store(true, std::memory_order_release); } } if(pIdx==1) fxParams[2][0]=1000.0f+(norm*10000.0f); if(pIdx==2) fxParams[2][1]=1.0f+(norm*100.0f); if(pIdx==3) fxParams[2][2]=0.005f+(norm*0.045f); }
        else if(currentMode==3) { if(pIdx==0) { effectMemory[3]=roundf((norm*48.0f)-24.0f); lutNeedsUpdate=true; forceUIUpdate.store(true, std::memory_order_release); } if(pIdx==1) fxParams[3][0]=norm; }
        else if(currentMode==4) { if(pIdx==0) { int cents=(int)roundf((effectMemory[4]-(float)roundf(effectMemory[4]))*100.0f); effectMemory[4]=constrain(roundf((norm*48.0f)-24.0f)+((float)cents/100.0f),-24.0f,24.0f); lutNeedsUpdate=true; forceUIUpdate.store(true, std::memory_order_release); } if(pIdx==1) { int semi=(int)roundf(effectMemory[4]); float c=roundf((norm*100.0f)-50.0f)/100.0f; effectMemory[4]=constrain((float)semi+c,-24.0f,24.0f); lutNeedsUpdate=true; forceUIUpdate.store(true, std::memory_order_release); } }
        else if(currentMode==5) { if(pIdx==0) { effectMemory[5]=roundf((norm*48.0f)-24.0f); lutNeedsUpdate=true; forceUIUpdate.store(true, std::memory_order_release); } if(pIdx==1) fxParams[5][0]=0.01f+(norm*0.5f); if(pIdx==2) fxParams[5][1]=0.001f+(norm*0.05f); if(pIdx==3) fxParams[5][2]=0.1f+(norm*0.8f); if(pIdx==4) fxParams[5][3]=norm; }
        else if(currentMode==6) { if(pIdx==0) { effectMemory[6]=roundf((norm*48.0f)-24.0f); lutNeedsUpdate=true; forceUIUpdate.store(true, std::memory_order_release); } if(pIdx==1) fxParams[6][0]=0.8f+(norm*0.199f); if(pIdx==2) fxParams[6][1]=norm*3.0f; }
        else if(currentMode==7) { if(pIdx==0) { effectMemory[7]=roundf((norm*48.0f)-24.0f); lutNeedsUpdate=true; forceUIUpdate.store(true, std::memory_order_release); } if(pIdx==1) fxParams[7][0]=500.0f+(norm*4500.0f); if(pIdx==2) fxParams[7][1]=norm; }
        else if(currentMode==8) { if(pIdx==0) { effectMemory[1]=roundf((norm*48.0f)-24.0f); lutNeedsUpdate=true; forceUIUpdate.store(true, std::memory_order_release); } if(pIdx==1) { effectMemory[0]=roundf((norm*48.0f)-24.0f); lutNeedsUpdate=true; forceUIUpdate.store(true, std::memory_order_release); } if(pIdx==2) fxParams[8][0]=0.001f+(norm*0.05f); if(pIdx==3) fxParams[8][1]=0.00001f+(norm*0.001f); if(pIdx==4) fxParams[8][2]=0.00001f+(norm*0.0005f); }
        else if(currentMode==9) { if(pIdx==0) { effectMemory[9]=roundf((norm*48.0f)-24.0f); lutNeedsUpdate=true; forceUIUpdate.store(true, std::memory_order_release); } if(pIdx==1) fxParams[9][0]=norm*2.0f; }
...
void MidiTask(void * pvParameters) {
    ...
    for(;;) {
        ...
        if(currentBtState!=lastBtState) { lastBtState=currentBtState; forceUIUpdate.store(true, std::memory_order_release); if(isScreenOff.load(std::memory_order_acquire)) turnScreenOn(); lastActivityTime=millis(); lastScreenActivityTime=millis(); }
        ...
        if(digitalRead(BOOT_SENSE_PIN)==HIGH) {
            ...
            if(moveA||moveB||moveC) {
                ...
                if(pitchChanged) { if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY); pitchShiftFactor.store(pitchShiftLUT[constrain(lastActivePedal,0,16383)], std::memory_order_release); if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); }
            ...
            if(!wasBypassed) { pedals.resetToCenter(); lastActivePedal=8192; if(!lutNeedsUpdate) { if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY); pitchShiftFactor.store(pitchShiftLUT[8192], std::memory_order_release); if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); } if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY); if(isVolumeMode) volumePedalGain=1.0f; if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); forceUIUpdate.store(true, std::memory_order_release); wasBypassed=true; }
        }
        
        smoothedRawBat=(smoothedRawBat==0.0f)?(float)latestBat:(smoothedRawBat*0.95f)+((float)latestBat*0.05f);
        if(millis()-lastBatteryTime>1000) { lastBatteryTime=millis(); const float CALIBRATION_MULTIPLIER=1.04571f; float instantVoltage=(smoothedRawBat/4095.0f)*3.3f*2.0f*CALIBRATION_MULTIPLIER; if(instantVoltage>2.0f) { bool charging=(instantVoltage>4.20f); int newPercent=getBatteryPercentage(instantVoltage); bool stateChanged=(newPercent!=currentBatteryPercent)||(charging!=isBatteryCharging); currentBatteryVoltage=instantVoltage; currentBatteryPercent=newPercent; isBatteryCharging=charging; if(stateChanged) forceUIUpdate.store(true, std::memory_order_release); } }
        
        static unsigned long lastLutUpdate=0;
        if(lutNeedsUpdate && (millis()-lastLutUpdate>40)) { lutNeedsUpdate=false; updateLUT(); if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY); pitchShiftFactor.store(pitchShiftLUT[constrain(lastActivePedal,0,16383)], std::memory_order_release); if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); lastLutUpdate=millis(); }
...
bool channelMessageCallback(ChannelMessage cm) {
    ...
    if(cm.header==0xB0) {
        if(cm.data1==11) { uint16_t mappedCC=map(cm.data2,0,127,0,16383); currentCC11=mappedCC; currentPB3=mappedCC; lastActivePedal=mappedCC; if(isVolumeMode) { if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY); volumePedalGain=(float)mappedCC/16383.0f; if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); Control_Surface.sendControlChange({19,Channel_1},cm.data2); } else { if(!lutNeedsUpdate) { if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY); pitchShiftFactor.store(pitchShiftLUT[mappedCC], std::memory_order_release); if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); } } forceUIUpdate.store(true, std::memory_order_release); return false; }
        ...
        if(cm.data1==5 && cm.data2>=64) { if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY); isPB2WiperMode=!isPB2WiperMode; if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); pb2ToggleRequested=true; }
            ...
            if(anyEffectOn||activeEffectMode.load(std::memory_order_acquire)!=0) { isWhammyActive=false; isFrozen=false; isFeedbackActive=false; isHarmonizerMode=false; isCapoMode=false; isSynthMode=false; isPadMode=false; isChorusMode=false; isSwellMode=false; isVibratoMode=false; activeEffectMode.store(0, std::memory_order_release); panicResetRequested.store(true, std::memory_order_release); } else { isWhammyActive=true; } 
            ...
            if(!lutNeedsUpdate && pitchShiftLUT!=nullptr) pitchShiftFactor.store(pitchShiftLUT[8192], std::memory_order_release); 
            if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); 
            dsp_is_paused.store(false, std::memory_order_release);
            while(dsp_ack_parked.load(std::memory_order_acquire)) { vTaskDelay(pdMS_TO_TICKS(1)); }
        ...
        else if(cm.data1==8 && cm.data2>=64) { if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY); isFrozen=!isFrozen; if(activeEffectMode.load(std::memory_order_acquire)==1) isWhammyActive=isFrozen; if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); forceUIUpdate.store(true, std::memory_order_release); settingsNeedSaving=true; lastParameterChangeTime=millis(); }
        else if(cm.data1==9 && cm.data2>=64) { if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY); isFeedbackActive=!isFeedbackActive; if(activeEffectMode.load(std::memory_order_acquire)==2) isWhammyActive=isFeedbackActive; if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); forceUIUpdate.store(true, std::memory_order_release); settingsNeedSaving=true; lastParameterChangeTime=millis(); }
        else if(cm.data1==10 && cm.data2>=64) { if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY); isHarmonizerMode=!isHarmonizerMode; if(activeEffectMode.load(std::memory_order_acquire)==3) isWhammyActive=isHarmonizerMode; if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); forceUIUpdate.store(true, std::memory_order_release); settingsNeedSaving=true; lastParameterChangeTime=millis(); }
        else if(cm.data1==12 && cm.data2>=64) { if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY); isCapoMode=!isCapoMode; if(activeEffectMode.load(std::memory_order_acquire)==4) isWhammyActive=isCapoMode; if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); lutNeedsUpdate=true; forceUIUpdate.store(true, std::memory_order_release); settingsNeedSaving=true; lastParameterChangeTime=millis(); }
        else if(cm.data1==13 && cm.data2>=64) { if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY); isSynthMode=!isSynthMode; if(activeEffectMode.load(std::memory_order_acquire)==5) isWhammyActive=isSynthMode; if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); forceUIUpdate.store(true, std::memory_order_release); settingsNeedSaving=true; lastParameterChangeTime=millis(); }
        else if(cm.data1==14 && cm.data2>=64) { if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY); isPadMode=!isPadMode; if(activeEffectMode.load(std::memory_order_acquire)==6) isWhammyActive=isPadMode; if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); forceUIUpdate.store(true, std::memory_order_release); settingsNeedSaving=true; lastParameterChangeTime=millis(); }
        else if(cm.data1==15 && cm.data2>=64) { if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY); isChorusMode=!isChorusMode; if(activeEffectMode.load(std::memory_order_acquire)==7) isWhammyActive=isChorusMode; if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); forceUIUpdate.store(true, std::memory_order_release); settingsNeedSaving=true; lastParameterChangeTime=millis(); }
        else if(cm.data1==16 && cm.data2>=64) { if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY); isSwellMode=!isSwellMode; if(activeEffectMode.load(std::memory_order_acquire)==8) isWhammyActive=isSwellMode; if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); forceUIUpdate.store(true, std::memory_order_release); settingsNeedSaving=true; lastParameterChangeTime=millis(); }
        else if(cm.data1==7 && cm.data2>=64) { if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY); isVibratoMode=!isVibratoMode; if(activeEffectMode.load(std::memory_order_acquire)==9) isWhammyActive=isVibratoMode; if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); forceUIUpdate.store(true, std::memory_order_release); settingsNeedSaving=true; lastParameterChangeTime=millis(); }
        else if(cm.data1==17 || cm.data1==18) { float step=(cm.data2>=64)?-1.0f:1.0f; if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY); int renderMode=activeEffectMode.load(std::memory_order_acquire); if(cm.data1==17) { if(renderMode==0||renderMode==1||renderMode==8) { effectMemory[1]=constrain(effectMemory[1]+step,-24.0f,24.0f); } else if(renderMode==4) { effectMemory[4]=constrain(effectMemory[4]+step,-24.0f,24.0f); } else if(renderMode==2) { if(step>0) feedbackIntervalIdx.store((feedbackIntervalIdx.load(std::memory_order_acquire)+1)%5, std::memory_order_release); else feedbackIntervalIdx.store((feedbackIntervalIdx.load(std::memory_order_acquire)+4)%5, std::memory_order_release); } else { effectMemory[renderMode]=constrain(effectMemory[renderMode]+step,-24.0f,24.0f); } } else if(cm.data1==18) { if(renderMode==0||renderMode==1||renderMode==8) { effectMemory[0]=constrain(effectMemory[0]+step,-24.0f,24.0f); } else if(renderMode==4) { float centStep=step*0.01f; effectMemory[4]=constrain(effectMemory[4]+centStep,-24.0f,24.0f); } } if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex); lutNeedsUpdate=true; forceUIUpdate.store(true, std::memory_order_release); settingsNeedSaving=true; lastParameterChangeTime=millis(); }
    }
    return false;
}
...
void setup() {
    ...
    adc_continuous_handle_cfg_t adc_config={}; adc_config.max_store_buf_size=16384; 
    ...
    fbDelayBuffer=(int16_t*)heap_caps_aligned_alloc(64, FB_BUFFER_SIZE*sizeof(int16_t), MALLOC_CAP_SPIRAM);
    freezeBuffer=(int16_t*)heap_caps_aligned_alloc(64, FREEZE_BUFFER_SIZE*sizeof(int16_t), MALLOC_CAP_SPIRAM);
    pitchShiftLUT=(float*)heap_caps_aligned_alloc(64, 16384*sizeof(float), MALLOC_CAP_SPIRAM);
    ...
    inBuf = (float*)heap_caps_aligned_alloc(64, HOP_SIZE * sizeof(float), MALLOC_CAP_SPIRAM);
    envBuf = (float*)heap_caps_aligned_alloc(64, HOP_SIZE * sizeof(float), MALLOC_CAP_SPIRAM);
    fzOutBuf = (float*)heap_caps_aligned_alloc(64, HOP_SIZE * sizeof(float), MALLOC_CAP_SPIRAM);
    masterGainBuf = (float*)heap_caps_aligned_alloc(64, HOP_SIZE * sizeof(float), MALLOC_CAP_SPIRAM);
    w1Buf = (float*)heap_caps_aligned_alloc(64, HOP_SIZE * sizeof(float), MALLOC_CAP_SPIRAM);
    w2Buf = (float*)heap_caps_aligned_alloc(64, HOP_SIZE * sizeof(float), MALLOC_CAP_SPIRAM);
    w3Buf = (float*)heap_caps_aligned_alloc(64, HOP_SIZE * sizeof(float), MALLOC_CAP_SPIRAM);
    padFilterBuf = (float*)heap_caps_aligned_alloc(64, HOP_SIZE * sizeof(float), MALLOC_CAP_SPIRAM);
    dryBuf = (float*)heap_caps_aligned_alloc(64, HOP_SIZE * sizeof(float), MALLOC_CAP_SPIRAM);
    fbOutBuf = (float*)heap_caps_aligned_alloc(64, HOP_SIZE * sizeof(float), MALLOC_CAP_SPIRAM);
    sMixBuf = (float*)heap_caps_aligned_alloc(64, HOP_SIZE * sizeof(float), MALLOC_CAP_SPIRAM);
    ...
    if(audioBufferMutex!=NULL) xSemaphoreTake(audioBufferMutex, portMAX_DELAY);
    pitchShiftFactor.store(pitchShiftLUT[8192], std::memory_order_release);
    if(audioBufferMutex!=NULL) xSemaphoreGive(audioBufferMutex);
    ...
    xTaskCreatePinnedToCore(MidiTask, "Midi", 8192, NULL, 10, NULL, 1);
