// v1.11 Lilygo Release Controller (Dynamic Degradation Edition)
...
void DisplayTask(void * pvParameters) {
    ...
    for(;;) {
        ...
        if(!isScreenOff.load(std::memory_order_acquire)) {
            ...
            sprintf(buf, "%4.2fV %3d%%  ", currentBatteryVoltage, currentBatteryPercent.load(std::memory_order_relaxed));
            tft.drawString(buf, 4, 2, 2);
            
            sprintf(buf, "   %-10s   ", EFFECT_NAMES[renderMode]);
            tft.drawCentreString(buf, 160, 2, 2);
            
            if (!bleEnabled.load(std::memory_order_relaxed)) {
                sprintf(buf, "  BT: OFF ");
            } else {
                sprintf(buf, "  BT: %-4s", btmidi.isConnected() ? "CONN" : "WAIT");
            }
            tft.drawString(buf, 240, 2, 2);
            ...
            int pb1_pct = (int)std::roundf(((float)currentPB1 - 8192.0f) / 81.915f);
            int pb2_pct = (int)std::roundf(((float)currentPB2 - 8192.0f) / 81.915f);
            int pb3_pct = (int)std::roundf(currentPB3 / 163.83f);
            int cc11_pct = (int)std::roundf(currentCC11 / 163.83f);
            
            sprintf(buf, "%4d%%", pb1_pct); tft.drawCentreString(buf, cx_top[0], cy1-4, 1);
            sprintf(buf, "%4d%%", pb2_pct); tft.drawCentreString(buf, cx_top[1], cy1-4, 1);
            sprintf(buf, "%4d%%", pb3_pct); tft.drawCentreString(buf, cx_top[2], cy1-4, 1);
            sprintf(buf, "%4d%%", cc11_pct); tft.drawCentreString(buf, cx_top[3], cy1-4, 1);
            ...
            int fxX = 22;
            for(int i=0; i<10; i++) {
                tft.setTextColor(fxStates[i] ? TFT_BLACK : TFT_DARKGREY, fxStates[i] ? TFT_GREEN : TFT_BLACK);
                tft.drawString(fxNamesShort[i], fxX, 138, 1);
                fxX += 28;
            }

            tft.fillRect(0, 155, 320, 15, TFT_DARKGREY);
            tft.setTextColor(TFT_WHITE, TFT_DARKGREY);
            sprintf(buf, "CPU:%3d%%  SRM:%3dK  PSR:%4dK  SR:%2dkHz  ", 
                ...
                currentSampleRate.load(std::memory_order_acquire)/1000);
            tft.drawString(buf, 10, 156, 1);
        }
...
// --- ZERO WAIT-STATE 6-POINT WINDOWED SINC INTERPOLATION (PREMIUM) ---
inline float IRAM_ATTR __attribute__((hot)) __attribute__((always_inline)) __attribute__((optimize("O3"))) processSincTap(uint32_t tapPhase, const int16_t* sramBuffer, int currentSramWriteIdx, uint32_t windowMask, uint32_t hannIntMult) {
    ...
    float frac = (tapPhase & 0xFFFF) * 0.0000152587890625f; // [0.0, 1.0)
    int effTap = T + 3;
...
// --- DYNAMIC DEGRADATION 2-POINT LINEAR INTERPOLATOR (HIGH SPEED) ---
inline float IRAM_ATTR __attribute__((hot)) __attribute__((always_inline)) __attribute__((optimize("O3"))) processLinearTap(uint32_t tapPhase, const int16_t* sramBuffer, int currentSramWriteIdx, uint32_t windowMask, uint32_t hannIntMult) {
    int T = (tapPhase >> 16) & windowMask; 
    ...
    float interpSample = __builtin_fmaf(s_1 - s_0, frac, s_0);
        ...
        if(__builtin_expect(bytesRead>0, 1)) {
            ...
            if(__builtin_expect(framesRead == HOP_SIZE, 1)) {
                ...
                if(__builtin_expect(globalAudioResetRequested.load(std::memory_order_acquire), 0)) {
                    ...
                    synthEnv=0.0f; synthFilter=0.0f; padFilter=0.0f; padEnv=0.0f; inputEnvelope=0.0f; feedbackFilterVar=0.0f; smoothedVolGain=volumePedalGain; currentPitch=1.0f; freezeWriteIdxVar=0; freezePlayCounterVar=0; freezeStartIdxVar=0; activeFreezeLength=currentSampleRate.load(std::memory_order_acquire); fbDelayWriteIdx=0; writeIndex=0; sramWriteIdx=0; apfNeedsClear=true; input_dc_offset=0.0f; ui_audio_level.store(0.0f, std::memory_order_release); ui_output_level.store(0.0f, std::memory_order_release); freezeRamp=0.0f; feedbackRamp=0.0f; vibratoLfoPhase=0.0f; chorusLfoPhase=0.0f; feedbackLfoPhase=0.0f;
                    uint32_t halfWinFixed=((uint32_t)currentWindowSize/2)<<16; tap_w1_1=0; tap_w1_2=halfWinFixed; tap_w2_1=0; tap_w2_2=halfWinFixed; tap_w3_1=0; tap_w3_2=halfWinFixed; tap_w4_1=0; tap_w4_2=halfWinFixed; tap_w5_1=0; tap_w5_2=halfWinFixed;
                ...
                if(__builtin_expect(synthActive, 0)) for(int i=0; i<framesRead; i++) { 
                    ...
                    float fltCoeff=__builtin_fmaxf(0.001f,__builtin_fminf(0.99f,__builtin_fmaf(0.6f*synthEnv, srScale, p_sy_flt*srScale))); 
                    synthFilter=AntiDenormal(__builtin_fmaf(fltCoeff, (procSample-synthFilter), synthFilter+DC_OFFSET)); 
                ...
                for(int i=0; i<framesRead; i++) {
                    ...
                    sramPitchBuffer[sramWriteIdx] = sample16; // Dual-write into zero-wait-state Internal SRAM
                    ...
                    if(__builtin_expect(feedbackActive||localFbRamp>0.0f, 0)) {
                        ...
                        // DEGRADATION: w4 and w5 run the extremely fast Linear math instead of Sinc
                        float w4=processLinearTap(tap_w4_1,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult)+processLinearTap(tap_w4_2,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult);
                        float w5=processLinearTap(tap_w5_1,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult)+processLinearTap(tap_w5_2,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult);
                    ...
                    // PRIMARY PITCH: Gets the premium 6-point Sinc math
                    w1Buf[i]=processSincTap(tap_w1_1,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult)+processSincTap(tap_w1_2,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult); 
                    
                    // DEGRADATION: Secondary pitch channels drop to high-speed Linear math
                    w2Buf[i]=0.0f; if(__builtin_expect(harmActive, 0)) w2Buf[i]=processLinearTap(tap_w2_1,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult)+processLinearTap(tap_w2_2,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult); 
                    w3Buf[i]=0.0f; if(__builtin_expect(chorusActive, 0)) w3Buf[i]=processLinearTap(tap_w3_1,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult)+processLinearTap(tap_w3_2,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult);
                    ...
                    padFilterBuf[i]=padFilter; fbOutBuf[i]=fbOutNode; 
...
void setup() {
    ...
    if (!delayBuffer || !freezeBuffer || !i2s_in_block || !i2s_out_block || !pitchShiftLUT_temp || !inBuf || !fbDelayBuffer || !sramPitchBuffer) {
