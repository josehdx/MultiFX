// v1.9 Lilygo Release Controller (Production / Dynamic Bipolar UI Edition)
        ...
        if(__builtin_expect(bytesRead>0, 1)) {
            ...
            if(__builtin_expect(framesRead == HOP_SIZE, 1)) {
                ...
                if(__builtin_expect(synthActive, 0)) for(int i=0; i<framesRead; i++) { 
                    ...
                    int waveIdx=(int)((clampedProc+1.0f)*1023.5f) & 2047; // FIX: Strict bounds mask to prevent random array overruns
...
void setup() {
    ...
    if (!delayBuffer || !freezeBuffer || !i2s_in_block || !i2s_out_block || !pitchShiftLUT_temp || !inBuf || !fbDelayBuffer) {
...
void loop() {
    ...
    // FIX: Hard timeout to guarantee NVS saving even if ambient noise remains high
