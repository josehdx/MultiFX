// v1.10 Lilygo Release Controller (Internal SRAM Sinc Pitch Shifter Edition)
...
// --- ZERO WAIT-STATE 6-POINT WINDOWED SINC INTERPOLATION (SRAM ENGINE) ---
inline float IRAM_ATTR __attribute__((hot)) __attribute__((always_inline)) __attribute__((optimize("O3"))) processSincTap(uint32_t tapPhase, const int16_t* sramBuffer, int currentSramWriteIdx, uint32_t windowMask, uint32_t hannIntMult) {
    ...
    // Read 6 consecutive points directly from zero-wait-state Internal SRAM
    ...
    // 6-Point Lanczos / Sinc Windowing Kernel with FMA vector acceleration
    float fm1 = frac + 1.0f;
    float f0  = frac;
    float f1  = 1.0f - frac;
    float f2  = 2.0f - frac;
            ...
            if(__builtin_expect(framesRead == HOP_SIZE, 1)) {
                ...
                for(int i=0; i<framesRead; i++) {
                    ...
                    if(__builtin_expect(feedbackActive||localFbRamp>0.0f, 0)) {
                        ...
                        // Execute 6-Point Windowed Sinc Interpolation directly from SRAM
                        float w4=processSincTap(tap_w4_1,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult)+processSincTap(tap_w4_2,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult);
                        float w5=processSincTap(tap_w5_1,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult)+processSincTap(tap_w5_2,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult);
                    ...
                    // Zero Wait-State Pitch Taps reading Lanczos/Sinc interpolated values from SRAM
                    ...
                    w2Buf[i]=0.0f; if(__builtin_expect(harmActive, 0)) w2Buf[i]=processSincTap(tap_w2_1,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult)+processSincTap(tap_w2_2,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult); 
                    w3Buf[i]=0.0f; if(__builtin_expect(chorusActive, 0)) w3Buf[i]=processSincTap(tap_w3_1,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult)+processSincTap(tap_w3_2,sramPitchBuffer,sramWriteIdx,windowMask,hannIntMult);
...
void setup() {
    ...
    // Allocate Zero Wait-State Pitch Buffer directly in Internal SRAM (16KB)
