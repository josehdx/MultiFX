...
class AudioPipeline {
private:
    static inline float input_dc_offset = 0.0f, synthEnv = 0.0f, synthFilter = 0.0f, synthBandpass = 0.0f, padEnv = 0.0f, inputEnvelope = 0.0f, feedbackFilterVar = 0.0f, smoothedVolGain = 1.0f, currentPitch = 1.0f, fbOutNode = 0.0f, smoothed_delay_samples = 0.0f, dampState = 0.0f, wowState = 0.0f;
    static inline uint32_t wowRng = 123456789; 
    static inline bool wasFeedbackActive = false; 
    static inline int freezeWriteIdxVar = 0, freezePlayCounterVar = 0, freezeStartIdxVar = 0, activeFreezeLength = 96000;
    static inline float c_fx[10][5] = {0.0f}; 
    static inline int c_lat = 0, c_act = 0; 
    static inline bool c_w = true, c_fz = false, c_fb = false, c_hr = false, c_cp = false, c_sy = false, c_pd = false, c_ch = false, c_sw = false, c_vb = false; 
    static inline float c_vg = 1.0f;
    static constexpr float normFactor = 1.0f / 2147483648.0f;
    static constexpr float DC_OFFSET = 1e-9f;
    static inline float lastPadCutoff = -1.0f; 
    static inline int apf1Idx = 0, apf2Idx = 0;
    static inline float cross_lp1 = 0.0f, cross_lp2 = 0.0f;

public:
    static inline __attribute__((always_inline)) void handlePanicReset() {
    ...
    static inline __attribute__((always_inline)) void handleGlobalReset() {
        memset(activeDmaReadBuf, 0, HOP_SIZE * sizeof(int16_t)); memset(activeDmaWriteBuf, 0, HOP_SIZE * sizeof(int16_t));
        memset(fbDelayBuffer, 0, FB_BUFFER_SIZE * sizeof(int16_t));
    ...
    static inline __attribute__((always_inline)) void processBlock(int framesRead, DSPCoreState* activeDSP) {
        float srScale = 48000.0f / (float)currentSampleRate.load(std::memory_order_relaxed);
